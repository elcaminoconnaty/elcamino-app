...
export async function MealCoverageBanner({ departureId }: { departureId: string }) {
  ...
  if (!rows || rows.length === 0) {
    return (
      <Card className="bg-cream-50 border">
  ...
  if (missing.length === 0) {
    return (
      <Card className="border-green-200 bg-green-50">
        <CardContent className="py-2.5 px-4 text-sm text-green-900 flex items-center gap-2">
          <Coffee className="h-4 w-4" />
          <UtensilsCrossed className="h-4 w-4" />
    ...
    <Card className="border-amber-300 border-2 bg-amber-50">
      <CardContent className="py-3 px-4 text-sm">
        <div className="flex items-start gap-3">
          <div className="flex gap-1 text-amber-700 shrink-0 mt-0.5">
            <Coffee className="h-4 w-4" />
            <UtensilsCrossed className="h-4 w-4" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-medium text-amber-900 mb-1">Faltan comidas del grupo por reservar</div>
            <div className="flex flex-wrap gap-1.5">
              {missing.map((m, i) => (
                <span key={i} className="inline-flex items-center gap-1 bg-amber-100 text-amber-900 rounded-full px-2.5 py-1 text-xs">
